...
def job_process(script_path, logs_dir):
    """
    Create and submit a SLURM job directly using clusterjob for the ROADS task.
    """
    # Define script
    script = f"""
    ...
    print("Roads job submitted successfully!")
    ...
    if job_state == False:
        print("Roads job failed!")
        return False

    if job_state == True:
        print("Roads job done!")
        return True
